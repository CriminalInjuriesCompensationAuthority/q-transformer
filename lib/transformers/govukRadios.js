    ...
    if (opts.conditionalComponentMap) {
        ...
        */
        opts.conditionalComponentMap.forEach(mapping => {
            ...
            if (!item) {
                throw Error(
                    `No conditional component mapping found for item value: "${item.value}"`
                );
            }
            ...
            const components = mapping.componentIds.map(componentId => {
                ...
                component.macroOptions.label.isPageHeading = false;
                component.macroOptions.label.classes = component.macroOptions.label.classes.replace(
                    'govuk-label--xl',
                    ''
                );
