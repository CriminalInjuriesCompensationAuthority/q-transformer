...
function isCorrectOrder(questionSchema, previousQuestion) {
    const conditionalComponents = questionSchema?.options?.conditionalComponentMap;
    if (!conditionalComponents) {
        return true;
    }
    return conditionalComponents.every(component =>
        component.componentIds.every(dependentQuestion => {
            // Dependent questions can only be incorrectly added immediately before the question they depend on
            if (previousQuestion.id === dependentQuestion) {
                return false;
            }
            return true;
        })
            ...
            if (values.length > 0) {
                values.forEach((value, valueIndex) => {
                    ...
                    const questionSchema =
                        fullUiSchema[value.sectionId]?.options?.properties?.[value.id];
                    const previousQuestion = values[valueIndex - 1];
                    if (valueIndex === 0 || isCorrectOrder(questionSchema, previousQuestion)) {
