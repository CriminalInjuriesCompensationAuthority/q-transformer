        ...
        if (theme.type === 'theme') {
            ...
            if (values.length > 0) {
                values.forEach((value, index) => {
                    ...
                    // Now check if any dependent questions have already been pushed to summaryRow
                    fullUiSchema[value.sectionId]?.options?.properties?.[
                        value.id
                    ]?.options?.conditionalComponentMap?.forEach(conditionalQuestion => {
                        conditionalQuestion.componentIds.forEach(dependentQuestion => {
                            // Dependent questions can only be incorrectly added immediately before the question they depend on
                            if (index > 0 && values[index - 1].id === dependentQuestion) {
                                incorrectOrder = true;
                            }
                        });
                    });
