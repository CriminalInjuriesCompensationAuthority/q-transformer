...
function getDependantIdsForValue(value, fullUiSchema) {
    const {conditionalComponentMap} = fullUiSchema[value.sectionId]?.options?.properties?.[
        value.id
    ]?.options;

    if (conditionalComponentMap) {
        const dependantIds = conditionalComponentMap.flatMap(
            conditionalMapping => conditionalMapping.componentIds
        );
...
function summaryRowsIncludesDependant(dependantIds, values, currentValueIndex) {
    const lastValueIdPushedToSummaryRows = values[currentValueIndex - 1].id;

    if (dependantIds.includes(lastValueIdPushedToSummaryRows)) {
        return true;
    }
...
function insertSummaryRowBeforeDependant(summaryRow, summaryRows) {
    summaryRows.splice(-1, 0, summaryRow);
}
        ...
        if (theme.type === 'theme') {
            ...
            if (values.length > 0) {
                values.forEach((value, valueIndex) => {
                    ...
                    if (valueIndex > 0) {
                        const dependantIds = getDependantIdsForValue(value, fullUiSchema);
                        const valueHasDependants = dependantIds.length > 0;

                        if (
                            valueHasDependants &&
                            summaryRowsIncludesDependant(dependantIds, values, valueIndex)
                        ) {
                            return insertSummaryRowBeforeDependant(summaryRow, summaryRows);
                        }
                    }
