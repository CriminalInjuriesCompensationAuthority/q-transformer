...
module.exports = ({schemaKey, schema, options} = {}) => {
    ...
    summaryStructure.forEach(theme => {
        if (theme.type === 'theme') {
            if (theme.id !== 'default' && theme.title !== 'Answers') {
                // Drop-down help sections in allOfs count as a question and generate a default row so must be skipped
                const summaryRows = [];
                ...
                if (values.length > 0) {
                    values.forEach(value => {
                        ...
                        if (answer.value !== undefined) {
                            // Conditional questions within allOfs always exist so we must check if the question was answered
                            const summaryRow = {
                                key: {
                                    text: answer.label,
