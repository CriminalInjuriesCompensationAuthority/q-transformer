...
function propertyNunjucks(propertySchema, transformations) {
...
function buildComponentNunjucks(componentSchema, componentsSchema, transformations) {
    if ('properties' in componentSchema) {
        return propertyNunjucks(componentSchema, transformations);
    }
    ...
    if ('allOf' in componentSchema) {
        ...
        ${componentSchema.allOf
            .map(propertySchema => {
                return propertyNunjucks(propertySchema, transformations);
            })
            .join('\n')}
    {% endcall %}`;
    }
