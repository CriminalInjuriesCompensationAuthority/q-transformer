...
function sectionSchemaTransformations(
    ...
    function transformPropertySchema(acc, propertiesSchema, questionOrder) {
        ...
        Object.keys(properties).forEach(subSchemaKey => {
            ...
            questionOrder.push(subSchemaKey);
        });
        return acc;
    }
    if ('allOf' in sectionSchema) {
        ...
        const returnValue = sectionSchema.allOf.reduce((acc, propertiesSchema) => {
            return transformPropertySchema(acc, propertiesSchema, questionOrder);
        }, {});
        sectionQuestionOrder.push(questionOrder);
        return returnValue;
    }
    const questionOrder = [];
        ...
        ${schema.title ? `<h1 class="govuk-heading-xl">${schema.title}</h1>` : ''}
            ${sectionQuestionOrder
                .map((questionOrder, index) => {
                    ...
                    const questionsNunjucks = questionOrder
                        .map(questionId => {
                            const transformation = transformations[questionId];
                            if (
