...
function sectionSchemaTransformations(sectionSchema, options, transform, data, schemaErrors) {
    ...
    if ('allOf' in sectionSchema) {
        return sectionSchema.allOf.reduce((acc, propertiesSchema) => {
...
function orderSectionPropertyKeys(sectionSchemaCollection, order) {
    ...
    const sectionProperties = sectionSchemaCollection.map(sectionSchema => {
        if ('properties' in sectionSchema) {
            return Object.keys(sectionSchema.properties);
        }
        return allOfArrayToIdArray(sectionSchema.allOf);
    });

    // reorder
    // e.g. propertyIds: [[b, a], [c, d]], order: [a,b,c,d] >> output: [[a,b], [c,d]]
    sectionProperties.forEach((propertyIds, index) => {
        sectionProperties[index] = order.filter(propertyId => {
...
function allOf({schema, options, transform, data, schemaErrors} = {}) {
    ...
    const sectionPropertyOrder = orderSectionPropertyKeys(
        sectionSchemaCollection,
        opts.outputOrder
    );
