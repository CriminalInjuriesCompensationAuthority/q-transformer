...
function form({schemaKey, schema, options, transform, data, schemaErrors, fullUiSchema} = {}) {
    ...
    // Find the first sub schema with a title for the page
    const titleIndex = opts.outputOrder.findIndex(subSchema => {
        if (schema.properties[subSchema] && schema.properties[subSchema].title) {
            return true;
        }
        return false;
    });
    ...
    const transformations = opts.transformOrder.reduce((acc, subSchemaKey) => {
        ...
        if (!('title' in schema) && opts.outputOrder.indexOf(subSchemaKey) === titleIndex) {
            pageHeading = subSchema.title;
        ...
        if (schema.downloadSummary) {
            subSchema.downloadSummary = true;
        }
            ...
            ${opts.outputOrder
                .map(property => {
                    ...
                    if (
                        ...
                        transformation.componentName === 'govukTaskList'
                    ) {
