...
function form({schemaKey, schema, options, transform, data, schemaErrors, fullUiSchema} = {}) {
    ...
    let pageTitle;
    // if the schema has a title in it, then use that.
    if (schema.title) {
        // errors?
        pageTitle =
            Object.keys(schemaErrors).length === 0 && schemaErrors.constructor === Object
                ? ''
                : 'Error: ';
        ...
        transformation: `${summaryText}
            ...
            ${opts.outputOrder
                .map(property => {
                    ...
                    // if there is no pageTitle yet, then the schema.title doesn't exist.
                    // you the question title of the first item in the output order instead.
                    if (!pageTitle) {
                ...
                .join('\n')}`,
        pageTitle: `${pageTitle} - claim criminal injuries compensation - GOV.UK`
    };
}
