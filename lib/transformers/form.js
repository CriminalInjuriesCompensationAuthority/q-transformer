...
function form({schema, options, transform, data, schemaErrors} = {}) {
    let opts = {};
    ...
    const errorSummary = []; // format the errors for the summary
    const errors = Object.keys(schemaErrors);
    errors.forEach(error => {
        errorSummary.push({
            href: `#${error}`,
            text: schemaErrors[error]
        });
    });

    const nunjucksImports = getNunjucksImports(opts.outputOrder, transformations);

    const summaryText = errorSummary.length
        ? `{% from "error-summary/macro.njk" import govukErrorSummary %}
              {{ govukErrorSummary({
                titleText: "There is a problem",
                errorList:${JSON.stringify(errorSummary)}}) }}`
        : '';
