...
function form({schemaKey, schema, options, transform, data, schemaErrors, fullUiSchema} = {}) {
    ...
    let required = [];
    const requiredKeys = function(obj) {
        Object.keys(obj).forEach(key => {
            if (key === 'required' && Array.isArray(obj[key])) {
                required = [...new Set([...required, ...obj[key]])];
    ...
    const transformations = opts.transformOrder.reduce((acc, subSchemaKey) => {
        ...
        if (!requiredKeys(schema).includes(subSchemaKey)) {
            if ('title' in subSchema && 'type' in subSchema) {
                if (!('title' in schema) && opts.outputOrder.indexOf(subSchemaKey) === 0) {
                    subSchema.description =
                        'description' in subSchema
                            ? (subSchema.description += ' (optional)')
                            : '(optional)';
                } else {
                    subSchema.title += ' (optional)';
                }
            }
        }
