...
function simplifyUiSchema(uiSchema) {
    const orderArray = {};
    Object.keys(uiSchema).forEach(page => {
        if (uiSchema[page].options && uiSchema[page].options.outputOrder) {
            orderArray[page] = uiSchema[page].options.outputOrder;
        }
    });
    return orderArray;
}

function summaryFormatter(answerObject, options) {
    const orderArray = simplifyUiSchema(options);
    const answerIndex = {};
    if (answerObject) {
        Object.keys(answerObject).forEach(question => {
            ...
            if (answerObject[question] !== {}) {
                ...
                if (Object.keys(answerObject[question]).length === 1) {
                    ...
                    let answers = answerObject[question];
                    if (question in orderArray) {
                        answers = JSON.parse(
                            JSON.stringify(answerObject[question], orderArray[question], 4)
                        );
                    }
                    answerIndex[question].value = multipleAnswersFormat(answers); // A question with more than one answer appears in a multi-line format
                }
            }
        });
    }
