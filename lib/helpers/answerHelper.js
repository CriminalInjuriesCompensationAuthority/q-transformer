...
function summaryFormatter(answerObject, fullUiSchema) {
    ...
    Object.keys(answerObject).forEach(question => {
        answerIndex[question] = {
            href: `/apply/${removeSectionIdPrefix(question)}?next=check-your-answers`
        };
        if (answerObject[question] !== {}) {
            ...
            if (Object.keys(answerObject[question]).length === 1) {
                ...
                const uiSchema = fullUiSchema[question];
                if (uiSchema && uiSchema.options && uiSchema.options.outputOrder) {
                    answers = JSON.parse(JSON.stringify(answers, uiSchema.options.outputOrder));
                }
